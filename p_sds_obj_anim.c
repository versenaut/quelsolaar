...
boolean p_lod_anim_scale_update_test(PMesh *mesh, ENode *o_node)
{
	...
	if(mesh->anim.scale[0] != (egreal)scale[0] || mesh->anim.scale[1] != (egreal)scale[1] || mesh->anim.scale[2] != (egreal)scale[2])
	{
		mesh->anim.scale[0] = (egreal)scale[0];
		mesh->anim.scale[1] = (egreal)scale[1];
		mesh->anim.scale[2] = (egreal)scale[2];
		return TRUE;
	}
	return FALSE;
}
...
*/
void p_lod_anim_vertex_array(egreal *vertex, uint cv_count, PMesh *mesh, ENode *g_node)
{
	egreal *v, *data, x, y, z;
	uint i = 1, j;
	EGeoLayer *layer;
