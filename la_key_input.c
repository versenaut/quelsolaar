...
{
	fprintf(LAInputHandlerGlobal.file, "%f %f %u %u %u\n", output->pointer_x, output->pointer_y, output->mouse_button[0], output->mouse_button[1], output->mouse_button[2]);
}
...
void load_macro_input(BInputState *output, boolean cansle)
{
	...
	{
		if(sscanf(line, "%f %f %u %u %u", &output->pointer_x, &output->pointer_y, &a, &b, &c) != 5)
		{
			LAInputHandlerGlobal.play = FALSE;
			fclose(LAInputHandlerGlobal.file);
		}
	...
	output->last_mouse_button[0] = output->mouse_button[0];
	output->last_mouse_button[1] = output->mouse_button[1];
	output->last_mouse_button[2] = output->mouse_button[2];
	output->mouse_button[0] = a;
	output->mouse_button[1] = b;
	output->mouse_button[2] = c;
}
/*
typedef struct{
	uint	button;
...
void la_input_handeler(BInputState *output, InputData *data, uint screen_x, uint screen_y)
{
	...
	{
		output->click_pointer_x = output->pointer_x;
		output->click_pointer_y = output->pointer_y;
	}
}*/
