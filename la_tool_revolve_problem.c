...
void fill_to_end(double *matrix, uint *a, uint *b, uint start, uint skip, uint end, uint revolve, boolean side)
{
	...
	fprintf(stderr, "fill_to_end %u\n", start);
	while(start != end)
	{
		udg_destroy_edge(i / 2);
		fprintf(stderr, "fill_to_end1 %u\n", start);
		edge = udg_get_edge_data(&edge_count);
		crease = 0;
		crease2 = 0;
...
void la_t_revolve(uint *start_edge, uint revolve)
{
	...
	fprintf(stderr, "rev 1");
	for(i = 0; i < count && (edge[i * 2] != start_edge[0] || edge[i * 2 + 1] != start_edge[1]); i++);

//	for(i = 0; edge[i * 2] != start_edge[0] && edge[i * 2 + 1] != start_edge[1]; i++);
	if(i == count)
		return;
	fprintf(stderr, "rev 2");/*find_end(uint *edge, uint count, uint id, uint skip)*/
...
//return;
	...
	if(start_edge[0] == ends[0])
	{
		...
		for(i = 0; i < revolve; i++)
		{
			fprintf(stderr, "rev 6 %u %u %u\n", i, revolve, start_edge[1]);
			id = udg_find_empty_slot_polygon();
			udg_polygon_set(udg_find_empty_slot_polygon(), ends[0], a[i], a[(i + 1) % revolve], -1);
			udg_crease_set(id, 0, crease, 0, 0);
		}
		fprintf(stderr, "rev 6.5 %u\n", start_edge[1]);
		fill_to_end(matrix, a, b, start_edge[1], start_edge[0], ends[1], revolve, FALSE);
		fprintf(stderr, "rev 7 %u\n", start_edge[1]);
	}
	else if(start_edge[1] == ends[1])
	{
			fprintf(stderr, "rev 8");
		...
		if(udg_get_select(a[0]) > 0.1)
			...
			crease = 0;
		for(i = 0; i < revolve; i++)
		{	fprintf(stderr, "rev 9");

			id = udg_find_empty_slot_polygon();
			udg_polygon_set(udg_find_empty_slot_polygon(), ends[1], a[(i + 1) % revolve], a[i], -1);
			udg_crease_set(id, 0, crease, 0, 0);
		}
			fprintf(stderr, "rev 10");
		fill_to_end(matrix, a, b, start_edge[0], start_edge[1], ends[0], revolve, FALSE);
	}	
	else
	{
			fprintf(stderr, "rev 11");
		fill_vertex_revolve(matrix, a, start_edge[0], revolve);
		for(i = 0; i < revolve; i++)
			c[i] = a[i];
			fprintf(stderr, "rev 12");
		fill_to_end(matrix, a, b, start_edge[0], start_edge[1], ends[0], revolve, FALSE);
			fprintf(stderr, "rev 13");
		fill_vertex_revolve(matrix, a, start_edge[1], revolve);
	fprintf(stderr, "rev 14");
		if(udg_get_select(a[0]) > 0.1)
			crease = -1;
		else
			crease = 0;
...
void create_tube_matrix(double *matrix, double *vector, uint a, uint b, uint c)
{
	...
	point[1] += origo[1];
	point[2] += origo[2];
	create_matrix_normalized(matrix, origo, point, vector);
}
