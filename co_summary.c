...
#define SUMARY_COLOR 0.5

float co_draw_node_sumary(ENode *node, float x, float y, uint recursion)
{
	...
	sui_draw_text(x, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_ns_get_node_name(node), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
	line -= 0.02;
	...
	for(i = e_ns_get_next_tag_group(node, 0); i != (uint16)-1; i = e_ns_get_next_tag_group(node, i + 1))
	{
		if(content == FALSE)
		{
			sui_draw_text(x, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, "Tags:", SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
			line -= 0.02;
			content = TRUE;
		}
		sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_ns_get_tag_group(node, i), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
		line -= 0.02;
		for(j = e_ns_get_next_tag(node, i, 0); j != (uint16)-1; j = e_ns_get_next_tag(node, i, j + 1))
		{
			...
			sui_draw_text(x + 0.04, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, text, SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
			line -= 0.02;
		}
	}
	...
	switch(e_ns_get_node_type(node))
	{
		case V_NT_OBJECT :
			{
				sui_draw_text(0.2, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, "Links:", SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
				line -= 0.02;
				for(link = e_nso_get_next_link(node, 0); link != NULL; link = e_nso_get_next_link(node, e_nso_get_link_id(link) + 1))
				{
					...
					if((link_node = e_ns_get_node(0, e_nso_get_link_node(link))) != NULL)
					{
						...
						sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, text, SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
						line -= 0.02;
						if(recursion != 0)
							line -= co_draw_node_sumary(link_node, x + 0.04, line, recursion - 1);
					}
				}
				...
				for(i = e_nso_get_next_method_group(node, 0); i != (uint16)-1 ; i = e_nso_get_next_method_group(node, i + 1))
				{
					if(content == FALSE)
					{
						sui_draw_text(x, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, "Methods:", SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
						line -= 0.02;
						content = TRUE;
					}
					sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_nso_get_method_group(node, i), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
					line -= 0.02;
					for(j = e_nso_get_next_method(node, i, 0); j != (uint16)-1; j = e_nso_get_next_method(node, i, j + 1))
					{
						sui_draw_text(x + 0.04, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_nso_get_method(node, i, j), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
						line -= 0.02;
					}
				}
			}
		...
		case V_NT_GEOMETRY :
			{
				...
				for(layer = e_nsg_get_layer_next(node, 0); layer != NULL; layer = e_nsg_get_layer_next(node, e_nsg_get_layer_id(layer) + 1))
				{
					if(content == FALSE)
					{
						sui_draw_text(x, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, "layers:", SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
						line -= 0.02;
						content = TRUE;
					}
					sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_nsg_get_layer_name(layer), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
					line -= 0.02;
				}
			}
		break;
		case V_NT_MATERIAL :
			{
				...
				for(i = e_nsm_get_fragment_next(node, 0); i != (VNMFragmentID)-1; i = e_nsm_get_fragment_next(node, i + 1))
				{
					if(e_nsm_get_fragment(node, i) == VN_M_FT_OUTPUT)
					{
						...
						sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, frag->output.label, SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
						line -= 0.02;
					}
				}
			}
		...
		case V_NT_BITMAP :
			{
				...
				sui_draw_text(x, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, text, SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
				line -= 0.02;
				for(layer = e_nsb_get_layer_next(node, 0); layer != NULL; layer = e_nsb_get_layer_next(node, e_nsb_get_layer_id(layer) + 1))
				{
					...
					sui_draw_text(x + 0.02, line, SUI_T_SIZE * 0.7, SUI_T_SPACE * 0.7, e_nsb_get_layer_name(layer), SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
					line -= 0.02;
				}
			}
		break;
...
void co_draw_sumary(ENode *node, float x, float y)
{
	glPushMatrix();
	glTranslatef(-x, -y, -1);
	sui_draw_2d_line_gl(0.03, 0.015, 0.18, 0.09, SUMARY_COLOR, SUMARY_COLOR, SUMARY_COLOR);
	co_draw_node_sumary(node, 0.2, 0.1, 10);
	glPopMatrix();
