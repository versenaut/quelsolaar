...
boolean p_array_update(PGeoParams *p, PMesh *mesh, uint32 geometry, uint32 material)
{
	...
	for(i = 0; i < p->count; i++)
	{
		...
		for(j = 0; j < 3; j++)
		{
			...
			if(layer != NULL && (p->data[i].version[j] != e_nsg_get_layer_version(layer) || p->data[i].compleat[j] < e_nsg_get_polygon_legnth(g_node)))
			{
				if(p->data[i].version[j] != e_nsg_get_layer_version(layer))
					p->data[i].compleat[j] = 0;
				p->data[i].compleat[j] = p_create_material_array(mesh, p->arrays[i], e_nsg_get_layer_data(g_node, e_nsg_get_layer_by_id(g_node, 1)), e_nsg_get_polygon_legnth(g_node), e_nsg_get_vertex_legnth(g_node), j, e_nsg_get_layer_data(g_node, layer), e_nsg_get_layer_type(layer), p->data[i].compleat[j]);
