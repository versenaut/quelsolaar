...
void create_param_polygon(ENode *g_node, egreal *output, PTessTableElement *table, EGeoLayer *layer, uint32 *ref, uint channel, uint polygon, boolean quad)
{
	...
	switch(e_nsg_get_layer_type(layer))
	{
		...
		case VN_G_LAYER_POLYGON_CORNER_UINT32 :
		{
			...
			a = (egreal)layer_pos[0];
			b = (egreal)layer_pos[1];
			c = (egreal)layer_pos[2];
			j = 0;
			if(quad)
			{
				d = (egreal)layer_pos[3];
				for(i = 0; i < table->vertex_count; i++)
				{
					f = table->vertex_influence[j++] * a;
					f += table->vertex_influence[j++] * b;
...
uint create_param_segment(ENode *g_node, egreal *output, uint output_start, PTessTableElement **table_array, EGeoLayer *layer, uint32 *order_node, uint channel, uint start, uint tri_start, uint end)
{
	...
	switch(e_nsg_get_layer_type(layer))
	{
		...
		case VN_G_LAYER_VERTEX_UINT32 :
		{
			...
			{
				table = table_array[i];
				a = (egreal)layer_data[ref[order_node[i] * 4]];
				b = (egreal)layer_data[ref[order_node[i] * 4 + 1]];
				c = (egreal)layer_data[ref[order_node[i] * 4 + 2]];
				d = (egreal)layer_data[ref[order_node[i] * 4 + 3]];
				k = 0;
				for(j = 0; j < table->vertex_count; j++)
				{
					f = table->vertex_influence[k++] * a;
...
void p_layer_param_create_old(ENode *g_node, PMesh *mesh)
{
	...
	if(mesh->sub_stages[0] == 1)
	{
		...
		mesh->sub_stages[1]++;
		if(mesh->sub_stages[1] % 3 * mesh->render.mat_count == 0)
			p_ra_set_array_real(&mesh->param.array[param], (egreal *)mesh->temp, mesh->render.vertex_count);
