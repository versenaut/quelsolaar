...
void e_vc_disconnect(uint connection, const char *bye)
{
	...
	//	verse_send_connect_terminate(bye);
		...
		foreach_remove_dlut(&ENSGlobal.context[connection].look_up_table, e_ns_del_node, NULL);
		init_dlut(&ENSGlobal.context[connection].look_up_table);
...
void callback_send_node_destroy(void *user_data, VNodeID node_id)
{
	...
	if(node != NULL)
	{
		...
		remove_entry_dlut(&ENSGlobal.context[ENSGlobal.curent_net_connection].look_up_table, node_id);
...
void e_ns_init_head(ENodeHead *head, VNodeType type, VNodeID node_id, VNodeOwner owner)
{
	...
	add_entry_dlut(&ENSGlobal.context[ENSGlobal.curent_net_connection].look_up_table, node_id, head);
...
void e_ns_head_copy(ENodeHead *node_from, ENodeHead *node_to, VNodeType type)
{
	*node_to = *node_from;
	...
	add_entry_dlut(&ENSGlobal.context[ENSGlobal.curent_net_connection].look_up_table, node_to->node_id, node_to);
	free(node_from);
}
...
{
	return find_dlut(&ENSGlobal.context[ENSGlobal.curent_net_connection].look_up_table, node_id);
}
...
void enough_init(void)
{
	...
	for(i = 0; i < MAX_CONNECTIONS; i++)
	{
		...
		init_dlut(&ENSGlobal.context[i].look_up_table);
...
{
	return find_dlut(&ENSGlobal.context[connection].look_up_table, ENSGlobal.context[connection].selected_node[type]);
}
